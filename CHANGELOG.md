...
- **`winsorize` feature capping now applies when predicting, not only when training.** `--feature-outliers winsorize` (CLI) and `feature_outlier_policy="winsorize"` (MCP) capped each numeric column in the CSV before training, but the caps were never saved with the model. Every prediction used raw values, so any spec beyond its training cap went straight into the model. Tree models barely notice, but a linear model extrapolates without limit. In a synthetic test where true prices top out near 25, a Ridge model priced a far-out part at **634,843**. On an external time-series study the same setting drove out-of-sample R² to **−17.5**. The caps were also computed on all rows before the train/holdout split, so the holdout had been capped too and the quality metrics hid the problem.
  - The caps now live inside the saved pipeline, as a clipping step at the front of numeric preprocessing (`FeatureClipper`). They are learned from the training rows only (per fold during cross-validation) and applied on every predict path: CLI, MCP, and direct `model.predict` calls from the Python API. Explanations, likely ranges and what-if work unchanged.
  - Holdout metrics for winsorized models are now scored on uncapped rows, so they can read worse than before. The new numbers are the accurate ones.
  - The rescue measured in `research/feature_outlier_visibility.md` still holds: with one absurd cell, `winsorize` still keeps Ridge in `auto_train`'s selection (holdout MAE 1.48, versus 3.70 without capping).
  - **Models trained with `winsorize` before v1.1.1 still predict on raw values. Retrain them.** Models trained with any other policy are unaffected.
  - Feature diagnostics and auto feature ranking still run on the capped copy, so the selected features don't change.

### Changed
- **Likely-range wording no longer overstates the guarantee, and `--time-column` training now warns about it.** The `intervals` docstring and `TECHNICAL.md` described coverage as "mathematically guaranteed". It holds on average, and only when new rows resemble the holdout. Time-ordered data usually breaks that: in the same study, nominal 80% ranges covered 75% of days and 41% of the worst 10%. `p2predict-train --time-column` now prints a warning, and `--json` output gains an additive top-level `warnings` list that carries it.
- **`p2predict train` now points to `p2predict-train`.** Previously it failed with a generic usage error.
