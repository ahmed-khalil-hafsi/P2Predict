...
| `bias_gate_materiality.{md,py,json}` | **Open** — the `unreliable` verdict tracks holdout size, not bias magnitude: same model and same bias, flagged 10% of the time at n=50 and 100% at n=16,000. Proposes an effect-size floor, a log-space test, and `unknown` instead of a small-n free pass. |
