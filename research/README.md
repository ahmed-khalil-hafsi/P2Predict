...
| `small_n_conformal.{md,py,json}` | **Open** — the likely-range is calibrated on 10–60 rows at real catalog size, and below 90 parts the conformal quantile degenerates to the single largest residual. Proposes an opt-in out-of-fold calibration path. |
| `selection_noise.{md,py,json}` | **Open** — auto-mode picks the model family by argmax over a 3-fold CV; reshuffling the folds alone changes the winner. Proposes surfacing the uncertainty, not changing the rule. |
| `learning_curve.{md,py,json}` | **Open** — nothing tells a buyer whether more parts would help. Tests whether the error-vs-catalog-size curve can be honestly extrapolated from the rows they already have. |
| `small_n_harness.py` | Shared support for the three small-n studies above — turns a case-study CSV into a simulator for a 50–300 part catalog with a disjoint sample to grade against. |
