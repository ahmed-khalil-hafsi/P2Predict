...
| File | Status |
|---|---|
| `large_data_scalability.{md,py,txt}` | **Open** — P2Predict is documented and sized around the typical 50–300 part category, but should work equally well at hundreds of thousands of rows. The maths already does; the agent-facing plumbing does not (a 100k-row `predict_from_csv` returns ~37 MB of JSON). Proposes capped payloads, subsampled feature ranking, and a band count that grows with the calibration set. |
