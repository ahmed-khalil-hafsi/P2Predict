...
| File | Status |
|---|---|
| `feature_outlier_visibility.{md,py,txt}` | **Open — plumbing, not capability.** One extreme-magnitude cell in one numeric column sends Ridge's CV R² to −4.6e53, removing it from `auto_train` selection entirely; on data where Ridge is the *correct* model the delivered alternative is 2.4× worse on holdout (MAE 1.49 → 3.64), with nothing said to the user. The existing `feature_outlier_policy` fixes it completely (`drop` and `winsorize` both restore MAE 1.49/1.48 and the right model), so no new capability is needed. The gap is that `apply_feature_outlier_policy` returns a per-column summary which `cli/train.py:292` reports and **both MCP paths discard into `_`** (`server.py:224`, `server.py:994`) — so on the primary interface the agent is never told there is anything to act on, which is why agents never set the policy. Proposes surfacing the summary in `propose_training_plan` first; explicitly does *not* propose changing the default. |
