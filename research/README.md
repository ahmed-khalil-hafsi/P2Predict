...
| File | Status |
|---|---|
| `feature_outlier_visibility.{md,py,txt}` | **Fix implemented, open for review.** One extreme-magnitude cell in a numeric column sends Ridge's CV R² to −4.6e53 and removes it from `auto_train` selection; where Ridge is the *correct* model the delivered alternative is 2.4× worse (MAE 1.49 → 3.64). Not a code bug: `StandardScaler` is applied correctly, it is linear extrapolation plus selection-by-CV-R². The existing `feature_outlier_policy` already fixes it; the gap was that the agent was never told. `propose_training_plan` never inspected feature magnitude at all, and `train` bound the summary to `_`, while `cli/train.py:292` reported it. Two candidate fixes died on measurement (raw IQR counts flag 20–25% of rows on 50–300-row data; an IQR z-threshold false-fired on a near-constant column and failed to transfer — a real column at z=502 is unaffected). Shipped instead: a magnitude ratio with `EXTREME_MAGNITUDE_RATIO = 1e6`, ~5,900× above the worst real column measured and silent on all five case-study samples, surfaced as a question in `propose_training_plan` and a warning plus `feature_data_quality` in `train`. Default stays `warn`. |
