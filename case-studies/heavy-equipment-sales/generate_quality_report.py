"""Generate the procurement-style model-quality PDF report for the resale model.

This calls P2Predict's built-in ``plotting.plot_results_pdf`` on the trained
model and writes a multi-page PDF to ``assets/``. It also shells out to
``sips`` (macOS) to produce PNG previews of each page so they can be
embedded inline in the README.
...
def _latest_model() -> Path:
    ...
    if not candidates:
        sys.exit(
            f"No *_sale_price_usd_*.model in {MODELS_DIR}. Train one first — "
            "see the case study README's Reproducing section."
        )
    return candidates[-1]


def _rebuild_test_set(loaded: dict) -> tuple:
    """Re-derive the same train/test split P2Predict used at training time.

    The saved model carries the calibration residuals but not the test set
    itself. We re-load the same cleaned CSV through ``load_csv_file`` (what
    the train CLI does), apply the same outlier policies (``warn`` on both,
    keeping every row), and split with the same ``test_size=0.2`` default,
    so the holdout matches the model's reported metrics.
